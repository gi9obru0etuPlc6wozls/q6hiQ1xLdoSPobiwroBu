...
\begin{abstract}
\input{project_name} performs database schema version control and scaffold generation for Postgres databases and the Treefrog C++ MVC framework using YAML to 
\end{abstract}
...
\begin{tabular}{|p{.9\textwidth}|}
	...
	\rowcolor{LightRed}
	\begin{center}
		\input{project_name} is very much a work in progress at this time and is not ready for prime time. Use at your own risk.
	\end{center}\\
	\hline
\end{tabular}
\\
...
\input{project_name} is tool that can generate code to be used as final source files and/or migrations based on serial numbers and up/down direction and can execute programs. \input{project_name} uses the notion of "serial" numbers and "direction" to determine whether a migration "action" should be executed.

\section{Getting Started}

\subsection{Required Libraries}
\input{project_name} utilizes the Inja\cite{inja} template engine. Inja and \input{project_name} both rely on the nlohmann JSON library\cite{nlohmann}. \input{project_name} also uses the jbeder YAML\cite{jbeder} library.
...
\subsection{Building \protect\input{project_name} }
The following steps will compile the \input{project_name} tool. 
\begin{figure}[H]
	\begin{lstlisting}
	  mkdir build
	  cd build
...
All settings for \protect\input{project_name} are configured using YAML and is located in the config.yaml file. This file can be found in the "config" directory by default. The location of the file may be overridden by using -c or --config on the command line.
